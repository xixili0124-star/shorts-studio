...
const groups={files:{label:'라이브러리',view:'media',icon:'folder'},captions:{label:'자막',view:'captions',icon:'text'},sound:{label:'소리',view:'sounds',icon:'sound'},design:{label:'디자인',view:'quick-format',icon:'design'},tools:{label:'트래킹',view:'mosaic',icon:'tools'}};
const subviews={files:[],captions:[['styles','자막'],['create','자동자막'],['list','자막 목록']],sound:[['sounds','효과음'],['voice','AI TTS'],['silence','무음 컷']],design:[['quick-format','퀵포맷'],['templates','템플릿'],['graphics','그래픽'],['transitions','전환']],tools:[['mosaic','모자이크'],['crop-tracking','크롭']]};
...
export function desktopGroupForView(view){return ({media:'files',captions:'captions',sounds:'sound',voice:'sound',silence:'sound','quick-format':'design',templates:'design',graphics:'design',transitions:'design',mosaic:'tools','crop-tracking':'tools'})[view]||'files';}
export function desktopInspectorGroup(title,type){
 if(['내용','글자 스타일','변형','배치','원본 맞춤','오디오','원본 오디오','화면 설정 대상','속도'].includes(title))return 'basic';
...
export class DesktopStudio{
 ...
 mount(){
  ...
  // 상위 메뉴는 종류를 고르는 화면만 엽니다. 실제 추적 창은 하위 탭에서 명시적으로 선택합니다.
  nav.addEventListener('click',event=>{const target=event.target.closest('[data-desktop-group]');if(!target||this.hooks.busy())return;const group=target.dataset.desktopGroup,next=this.memory[group]||groups[group].view;this.hooks.setView(next);});
