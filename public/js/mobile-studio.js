...
export class MobileStudio{
 ...
 mount(){
  ...
  shell.innerHTML=`<header class="mobile-header">${iconButton('mobileProjectMenu','프로젝트 메뉴','folder')}<div class="mobile-project-title"><strong id="mobileProjectTitle"></strong><small id="mobileSaveStatus">자동 저장</small></div><button type="button" id="mobileExport">완성 ${icon('save')}</button></header><div id="mobileViewerHost"></div><section id="mobileEditDeck" aria-label="모바일 타임라인"><div class="mobile-edit-heading"><h2>타임라인</h2><button type="button" id="mobileOverview" aria-pressed="false">${icon('tracks')}<span>전체 트랙</span></button><div class="mobile-history">${iconButton('mobileUndo','실행 취소','undo')}${iconButton('mobileRedo','다시 실행','redo')}</div></div><nav id="mobileLayerBar" aria-label="편집할 트랙"></nav><div id="mobileTimelineHost"></div><div id="mobileTimelineFooter"><span class="mobile-selection-name">클립을 눌러 편집</span><div class="mobile-zoom-controls"><button type="button" id="mobileZoomMinus" aria-label="타임라인 축소">−</button><button type="button" id="mobileZoomFit">맞춤</button><button type="button" id="mobileZoomPlus" aria-label="타임라인 확대">＋</button></div></div></section><nav id="mobileActionDock" aria-label="현재 작업 도구"></nav><div id="mobileParking" hidden></div>`;document.body.append(shell);
  ...
  $('mobileProjectMenu').onclick=()=>this.openMenu('project');$('mobileExport').onclick=()=>this.command('openExport');$('mobileUndo').onclick=()=>this.command('undo');$('mobileRedo').onclick=()=>this.command('redo');$('mobileZoomMinus').onclick=()=>this.command('zoomOut');$('mobileZoomPlus').onclick=()=>this.command('zoomIn');$('mobileZoomFit').onclick=()=>this.command('fitTimeline');
 ...
 syncSelection(){
  ...
  const selected=state.type&&state.type!=='asset';$('mobileTimelineFooter').querySelector('.mobile-selection-name').textContent=this.hooks.timeline.mobileMultiSelect?'클립을 눌러 선택 추가':selected?(state.count>1?state.count+'개 선택':state.name):'클립을 눌러 편집';
