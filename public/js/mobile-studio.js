...
export class MobileStudio{
 constructor(hooks){
  this.hooks=hooks;this.active=false;this.sheet=null;this.inspectorGroup='all';this.librarySection=null;this.trackId=null;this.menuPage='project';this.selectionKey='';this.filtered=new Set();
 ...
 mount(){
  ...
  shell.innerHTML=`<header class="mobile-header">${iconButton('mobileProjectMenu','프로젝트 메뉴','folder')}<div class="mobile-project-title"><strong id="mobileProjectTitle"></strong><small id="mobileSaveStatus">자동 저장</small></div><button type="button" id="mobileExport">내보내기 ${icon('save')}</button></header><div id="mobileViewerHost"></div><section id="mobileEditDeck" aria-label="모바일 타임라인"><div class="mobile-edit-heading"><h2>타임라인</h2>${iconButton('mobileTrackManager','트랙 관리','tracks')}<div class="mobile-history">${iconButton('mobileUndo','실행 취소','undo')}${iconButton('mobileRedo','다시 실행','redo')}</div></div><div id="mobileTimelineHost"></div></section><nav id="mobileActionDock" aria-label="현재 작업 도구"></nav><div id="mobileParking" hidden></div>`;document.body.append(shell);
  ...
  $('mobileTrackManager').onclick=()=>this.openMenu('tracks');
 ...
 syncTracks(){
  if(!this.active)return;const tracks=this.hooks.tracks(),normalized=this.hooks.timeline.setMobileTrackView?.({all:true,trackId:this.trackId})||{trackId:this.trackId||tracks.find(t=>t.role==='video')?.id};this.trackId=normalized.trackId;
  document.body.dataset.mobileTimeline='all';this.trackSignature=JSON.stringify(tracks.map(t=>[t.id,t.label]));
 }
 ...
 refreshPanel(){
  ...
  else if(this.sheet==='library'&&this.hooks.view()==='captions')for(const child of $('libraryContent').children)this.hide(child,this.librarySection==='automatic'?!child.matches('.smart-card'):this.librarySection==='caption-styles'?!(child.matches('.preset-grid,.segmented,[data-action="add-caption"]')||(child.matches('.section-label')&&child.textContent.startsWith('자막 스타일'))):false);
 }
 ...
 openSheet(name){
  ...
  $('mobilePanelTitle').textContent=name==='inspector'?groupLabels[this.inspectorGroup]:name==='library'?({media:'파일 추가',captions:this.librarySection==='automatic'?'자동 자막':'자막',graphics:'그래픽 추가',sounds:'효과음',voice:'AI TTS','quick-format':'퀵포맷',transitions:'장면 전환',mosaic:'모자이크','crop-tracking':'크롭 트래킹',silence:'무음 컷'}[this.hooks.view()]||'편집'):this.menuTitle;
  this.backPage=name==='menu'?({screen:'clip',decorate:'clip',track:'tracks'}[this.menuPage]||null):name==='library'&&['sounds','voice','silence'].includes(this.hooks.view())?'sound':name==='library'&&this.hooks.view()==='captions'?'captions':null;$('mobilePanelBack').hidden=!this.backPage;this.refreshPanel();$('mobilePanelBody').scrollTop=0;$('mobilePanelDone').focus({preventScroll:true});requestAnimationFrame(()=>{if(this.active)this.hooks.layout?.(true);});
 }
 ...
 renderMenu(){
  ...
  if(this.menuPage==='captions'){this.menuTitle='자막';html=row('텍스트 입력','text','new-caption','지금 위치에 새 자막')+row('자동 자막','voice','automatic','말소리를 자막으로')+row('자막','graphic','caption-styles','선택한 자막의 모양 바꾸기')+row('자막 파일 가져오기','folder','route:import-srt');}
  else if(this.menuPage==='sound'){this.menuTitle='소리';html=row('오디오 파일','folder','view:media','음악과 녹음 파일 가져오기')+row('효과음','sound','view:sounds','클릭, 등장, 알림 소리')+row('AI TTS','voice','view:voice','텍스트로 내레이션 만들기')+row('무음 컷','split','view:silence','말 없는 구간 정리');}
