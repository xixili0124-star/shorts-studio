...
export class MonitorEditor {
  constructor(callbacks){
    ...
    this.tools.addEventListener('click',e=>{
      ...
      if(mode){this.mode=mode.dataset.monitorMode;this.update();}
  ...
  update(){
    ...
    const polygons=entries.filter(r=>this.player.time>=r.start&&this.player.time<r.end).map(r=>{
      const points=visualCorners(this.bounds(r),r.item,W,H).map(screen);
    ...
    const corners=visualCorners(this.bounds(current),current.item,W,H).map(screen),mid=(a,b)=>({x:(a.x+b.x)/2,y:(a.y+b.y)/2});
    ...
    this.tools.querySelectorAll('[data-monitor-mode]').forEach(button=>{const active=button.dataset.monitorMode===this.mode;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active));});
  ...
  begin(event){
    ...
    if(!handle&&!hitVisual(p,bounds,current.item,W,H))return;
    this.player.pause();event.preventDefault();this.callbacks.gestureStart?.();
    const targets=entries.map(entry=>({entry,item:entry.item,bounds:this.bounds(entry),initial:{transform:clone(entry.item.transform),crop:clone(entry.item.crop)}}));
    const primary=targets.find(t=>t.entry.id===current.id),visible=croppedBounds(bounds,current.item.crop);
    const center=transformPoint({x:visible.x+visible.w/2,y:visible.y+visible.h/2},bounds,current.item,W,H);
  ...
  move(event){
    ...
    if(d.mode==='crop'){
      ...
      for(const target of d.targets){const t=transformOf(target.initial);target.item.transform={...t,offsetX:clamp(t.offsetX+snap.transform.offsetX-original.offsetX,-3,3),offsetY:clamp(t.offsetY+snap.transform.offsetY-original.offsetY,-3,3)};}
      ...
      for(const target of d.targets){const t=transformOf(target.initial),b=croppedBounds(target.bounds,target.initial.crop);target.item.transform=transformAroundAnchor(target.initial,target.bounds,W,H,{...t,rotation:wrapAngle(t.rotation+delta)},{x:b.x+b.w/2,y:b.y+b.h/2});}
    }else{
      ...
      for(const target of d.targets){
        ...
        target.item.transform=transformAroundAnchor(target.initial,target.bounds,W,H,{...t,scaleX:clamp(t.scaleX*rx,.05,10),scaleY:clamp(t.scaleY*ry,.05,10)},anchor);
      }
    }
  ...
  finish(cancel){
    ...
    const changed=d.targets.some(t=>!sameGeometry(t.initial,t.item));
    if(cancel||!changed)for(const target of d.targets){
      ...
      for(const key of ['transform','crop'])if(target.initial[key]===undefined)delete target.item[key];else target.item[key]=clone(target.initial[key]);
    }
