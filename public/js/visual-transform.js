...
import { evaluateItem, setValueAt } from './keyframes.js';
export function transformOf(item, localTime = 0) {
  const source = item.keyframes ? evaluateItem(item, localTime) : item;
  return { offsetX: 0, offsetY: 0, scaleX: 1, scaleY: 1, rotation: 0, opacity: 1, flipX: false, flipY: false, ...source.transform };
}
...
export function transformPoint(point, bounds, item, W, H, localTime = 0) {
  const t=transformOf(item,localTime),cx=bounds.x+bounds.w/2,cy=bounds.y+bounds.h/2,a=t.rotation*Math.PI/180;
...
export function visualCorners(bounds, item, W, H, localTime = 0) {
  const b=croppedBounds(bounds,item.crop);
  return [{x:b.x,y:b.y},{x:b.x+b.w,y:b.y},{x:b.x+b.w,y:b.y+b.h},{x:b.x,y:b.y+b.h}]
    .map(p=>transformPoint(p,bounds,item,W,H,localTime));
}
/** 화면 반전 후에도 원본의 왼쪽·오른쪽 경계를 구분하는 역변환입니다. */
export function inverseTransformPoint(point, bounds, item, W, H, localTime = 0) {
  const t=transformOf(item,localTime),sx=t.scaleX*(t.flipX?-1:1),sy=t.scaleY*(t.flipY?-1:1);
...
export function hitVisual(point,bounds,item,W,H,localTime=0) {
  const local=inverseTransformPoint(point,bounds,item,W,H,localTime),b=croppedBounds(bounds,item.crop);
  return !!local&&local.x>=b.x&&local.x<=b.x+b.w&&local.y>=b.y&&local.y<=b.y+b.h;
}
export function cropFromDrag(item,bounds,W,H,from,to,edges,localTime=0) {
  const a=inverseTransformPoint(from,bounds,item,W,H,localTime),b=inverseTransformPoint(to,bounds,item,W,H,localTime);
...
export function transformAroundAnchor(item,bounds,W,H,next,anchor,localTime=0) {
  const fixed=transformPoint(anchor,bounds,item,W,H,localTime);
...
export function resizeFromDrag(item,bounds,W,H,from,to,edges,uniform=true,localTime=0) {
  const t=transformOf(item,localTime),b=croppedBounds(bounds,item.crop);
  const a=inverseTransformPoint(from,bounds,item,W,H,localTime),p=inverseTransformPoint(to,bounds,item,W,H,localTime);if(!a||!p)return t;
  ...
  if(uniform&&horizontal&&vertical){
    const h=transformPoint(handle,bounds,item,W,H,localTime),q=transformPoint(anchor,bounds,item,W,H,localTime),x=h.x-q.x,y=h.y-q.y;
  ...
  return transformAroundAnchor(item,bounds,W,H,next,anchor,localTime);
}
export function snapVisualCenter(item,bounds,W,H,tolerance={x:8,y:8},localTime=0) {
  const tr=transformOf(item,localTime),corners=visualCorners(bounds,item,W,H,localTime);
...
export function alignVisual(item, bounds, W, H, axis, localTime=0, options={}) {
  const zero={...evaluateItem(item,localTime),transform:{...transformOf(item,localTime),offsetX:0,offsetY:0}};
  const corners=visualCorners(bounds,zero,W,H);
  const x=corners.reduce((n,p)=>n+p.x,0)/4,y=corners.reduce((n,p)=>n+p.y,0)/4;
  const channel=axis==='x'?'offsetX':'offsetY',value=axis==='x'?(W/2-x)/W:(H/2-y)/H;
  setValueAt(item,channel,localTime,clamp(value,-3,3),options);
}
export function withVisualTransform(ctx, bounds, item, W, H, paint, localTime=0) {
  const t=transformOf(item,localTime),cx=bounds.x+bounds.w/2,cy=bounds.y+bounds.h/2;
  ctx.save();
